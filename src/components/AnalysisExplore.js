...
const AnalysisExplore = () => {
  ...
  // Conjunto de datos
  const datasetGroup = {
    labels: varsSelected[0]
      ? varsSelected[0].tipo === "ordinal" || varsSelected[0].tipo === "nominal"
  ...
  const getDataUnits = () => {
    if (varsSelected[0].label.includes("peso")) {
      ...
      varsSelected[0].label.includes("talla") ||
      varsSelected[0].label.includes("pc")
    ) {
      setDataUnits(" (cm)");
  ...
  const getDatos = useCallback(async () => {
    if (
      ...
      if (
        ...
        const fields = await getValuesByVarYears(
          aniosSelected.map((anios) => anios.value),
          varsSelected[0].value
        );

        const stringArray = fields.map(String);
        setValores(stringArray);
        ...
        if (aniosSelected.length === 1) {
          ...
          // Mostrar gráfico agrupado por años
          const groupedData = await groupByYearsVarNumbers(
            aniosSelected.map((anios) => anios.value),
            varsSelected.map((vars) => vars.value)
          );
  ...
  const setRegisterCountText = () => {
    var count = 0;

    if (varsSelected.length > 0) {
      if (tipoData === "nominal" || tipoData === "ordinal") {
        if (aniosSelected.length === 1) {
          count = dataBar.reduce((a, b) => a + (b["y"] || 0), 0);
        } else {
          if (!varsSelected[0].value.includes(".")) {
            count = data.filter((obj) => obj[varsSelected[0].value] != null)
              .length;
          } else {
            count = data
              .filter((obj) => !!obj[varsSelected[0].value.split(".")[0]])
              .filter(
                (obj) =>
        ...
        if (aniosSelected.length === 1) {
          count = dataDist.reduce((a, b) => a + (b["y"] || 0), 0);
        } else {
          if (!varsSelected[0].value.includes(".")) {
            count = data.filter((obj) => obj[varsSelected[0].value] != null)
              .length;
              ...
              data && data.length > 0 ? (
                <div>
                  ...
                  <h6 className="text-start info-tag col-6">
                    ...
                    <b> {setRegisterCountText()} registros</b>
                  </h6>
                    ...
                    aniosSelected && aniosSelected.length === 1 ? (
                      ...
                      <div className="row pt-4 text-center pb-4">
                        ...
                        {/* <GroupedBar
                          ...
                          options={optionsGroup}
                        /> */}
                        <LineChart data={datasetGroup} options={optionsGroup} />
                      </div>
                    )
                  ) : tipoData === "continuo" ? (
                    aniosSelected && aniosSelected.length === 1 ? (
...
export default AnalysisExplore;
